...
pub const MAX_OUT_SESSIONS: usize = 12;

pub struct Manager {
    pub system: SharedSystem,
    pub session_ids: [usize; MAX_OUT_SESSIONS],
    pub applet_resource_user_ids: [usize; MAX_OUT_SESSIONS],
    pub sessions: Arc<Mutex<[Option<Arc<Out>>; MAX_OUT_SESSIONS]>>,
...
impl Manager {
    pub fn new(system: SharedSystem, audio_manager: Arc<AudioManager>) -> Self {
        Self {
            ...
            applet_resource_user_ids: [0; MAX_OUT_SESSIONS],
            sessions: Arc::new(Mutex::new(array::from_fn(|_| None))),
            num_free_sessions: MAX_OUT_SESSIONS,
            next_session_id: 0,
            free_session_id: 0,
    ...
    pub fn acquire_session_id(&mut self, session_id: &mut usize) -> Result {
        if self.num_free_sessions == 0 {
            error!("audio_core: all 12 AudioOut sessions are in use, cannot create any more");
            return RESULT_OUT_OF_SESSIONS;
        }
        *session_id = self.session_ids[self.next_session_id];
        self.next_session_id = (self.next_session_id + 1) % MAX_OUT_SESSIONS;
    ...
    pub fn release_session_id(&mut self, session_id: usize) {
        ...
        debug!("audio_core: freeing AudioOut session {session_id}");
        self.session_ids[self.free_session_id] = session_id;
        self.num_free_sessions += 1;
        self.free_session_id = (self.free_session_id + 1) % MAX_OUT_SESSIONS;
        self.set_session(session_id, None);
        self.applet_resource_user_ids[session_id] = 0;
    }

    pub fn set_session(&self, session_id: usize, session: Option<Arc<Out>>) {
    ...
    pub fn link_to_manager(&mut self) -> Result {
        ...
        if !self.linked_to_manager {
            ...
            let _ = self.audio_manager.set_out_manager(callback);
    ...
    pub fn get_audio_out_device_names(&self, names: &mut Vec<AudioDeviceName>) -> u32 {
        names.push(AudioDeviceName::from_str("DeviceOut"));
        1
    }
}
...
mod tests {
    ...
    use crate::out::audio_out::Out;
    use crate::out::audio_out_system::{
        AudioOutBuffer, AudioOutParameter, System as AudioOutSystem,
    };
    ...
    fn manager_callback_uses_live_session_table() {
        ...
        let mut out_system = AudioOutSystem::new(system.clone(), sink, buffer_event.clone(), 0);
        out_system.set_audio_manager(Some(audio_manager.clone()));
        assert!(out_system
            .initialize(
                String::new(),
                &AudioOutParameter {
            ...
            .is_success());
        assert!(out_system.start().is_success());
        assert!(out_system.append_buffer(
            AudioOutBuffer {
                next: 0,
                samples: 0,
                capacity: 8,
    ...
    fn start_links_manager_automatically() {
        ...
        let mut out_system = AudioOutSystem::new(system, sink, buffer_event.clone(), 0);
        out_system.set_audio_manager(Some(audio_manager.clone()));
        assert!(out_system
            .initialize(
                String::new(),
        ...
        let session = Arc::new(Out::new(out_system, buffer_event.clone(), Arc::new(|_| {})));
        ...
        manager.start();
        audio_manager.set_event(AudioEventType::AudioOutManager, true);
