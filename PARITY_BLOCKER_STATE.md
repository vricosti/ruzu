...
- Missing prerequisite: upstream `TextureCache<P>::RescaleRenderTargets` can rescale currently bound render targets before framebuffer lookup. Rust now ports the `BlitImage` `ImageCanRescale`/`ScaleUp`/`ScaleDown`/resolve-region slice in `video_core/src/renderer_opengl/gl_texture_cache.rs`, but render-target binding-time scale transitions and full descriptor-table invalidation remain incomplete.
- Additional prerequisite found while tracing the `Press L + R` source image: upstream `TextureCache<P>::JoinImages` runs the sibling-image rescale decision before creating/copying the joined image and uses the rescale factor in `MakeShrinkImageCopies`. Rust now applies that OpenGL backend portion while draining `PendingJoinCopies`, but `RefreshContents(new_image)` still cannot be run at the original upstream point unless the caller has a channel GPU reader.
- Required before resuming deeper ad-hoc rendering fixes: if the MK8D visual re-test still shows `Press L + R` corruption or cinematic overexposure and logs indicate rescaled render targets or stale views, port `RescaleRenderTargets` and the remaining `InvalidateScale` side effects next.
