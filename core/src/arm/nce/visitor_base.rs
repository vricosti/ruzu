...
impl Reg {
    ...
    pub const fn from_u32(v: u32) -> Self {
        ...
        if v > 31 { Self::R31 } else { unsafe { std::mem::transmute(v) } }
    }
...
impl Vec {
    pub const fn from_u32(v: u32) -> Self {
        if v > 31 { Self::V31 } else { unsafe { std::mem::transmute(v) } }
    }
...
pub trait VisitorBase {
    fn unallocated_encoding(&mut self) -> bool { false }

    // Loads and stores - Load/Store Exclusive (ordered)
    fn stllr(&mut self, size: Imm<2>, rn: Reg, rt: Reg) -> bool { false }
    fn stlr(&mut self, size: Imm<2>, rn: Reg, rt: Reg) -> bool { false }
    fn ldlar(&mut self, size: Imm<2>, rn: Reg, rt: Reg) -> bool { false }
    fn ldar(&mut self, size: Imm<2>, rn: Reg, rt: Reg) -> bool { false }

    // Loads and stores - Load register (literal)
    fn ldr_lit_gen(&mut self, opc_0: bool, imm19: Imm<19>, rt: Reg) -> bool { false }
    fn ldr_lit_fpsimd(&mut self, opc: Imm<2>, imm19: Imm<19>, vt: Vec) -> bool { false }
