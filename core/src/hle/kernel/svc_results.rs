...
//! Each constant is a ResultCode with ErrorModule::Kernel and the upstream description.
...
mod tests {
    ...
    #[test]
    fn test_kernel_result_codes_are_errors() {
        assert!(RESULT_OUT_OF_SESSIONS.is_error());
        assert!(RESULT_INVALID_ARGUMENT.is_error());
        assert!(RESULT_TIMED_OUT.is_error());
        assert!(RESULT_CANCELLED.is_error());
    }
    ...
    #[test]
    fn test_kernel_result_codes_description() {
        assert_eq!(RESULT_OUT_OF_SESSIONS.get_description(), 7);
        assert_eq!(RESULT_INVALID_ARGUMENT.get_description(), 14);
        assert_eq!(RESULT_TIMED_OUT.get_description(), 117);
        assert_eq!(RESULT_INVALID_ID.get_description(), 519);
    }
}
